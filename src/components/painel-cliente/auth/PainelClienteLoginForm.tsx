  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/painel-cliente/completar-cadastro`,
        },
      });
      if (error) {
        console.error('[PainelClienteLoginForm] ❌ Erro no login Google:', error);
      }
    } catch (err) {
      console.error('[PainelClienteLoginForm] ❌ Erro inesperado Google:', err);
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <Button
        type="button"
        variant="outline"
        className="w-full h-12 border-gray-300 hover:bg-gray-50 rounded-xl font-medium text-gray-700 transition-all flex items-center justify-center gap-3"
