    ...
    <div className="w-full h-full bg-white overflow-hidden">
      <div className="flex flex-col h-full">
        ...
        <div className="flex-shrink-0 p-3 sm:p-4 border-b border-gray-200">
          <h1 className="text-base sm:text-lg md:text-xl font-bold text-gray-900">Marketing</h1>
          <p className="text-xs sm:text-sm text-gray-600 mt-1">Campanhas, cupons e relatórios</p>
        </div>

        {/* Tabs Navigation - Compacta */}
        <div className="flex-shrink-0 px-3 sm:px-4 pt-2">
          <Tabs defaultValue="dashboard" value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid grid-cols-4 bg-gray-100 border border-gray-200 rounded-lg p-1 h-9 sm:h-10">
              <TabsTrigger 
                value="dashboard" 
                className="flex items-center gap-1 py-1 px-2 text-xs data-[state=active]:bg-gradient-to-r data-[state=active]:from-urbana-gold data-[state=active]:to-yellow-500 data-[state=active]:text-white text-gray-700 font-medium transition-all"
              >
