...
import { TrendingUp, TrendingDown, DollarSign, Users, ShoppingBag, Award } from 'lucide-react';
...
const RelatorioResumo: React.FC<Props> = ({ filters }) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { receitaTotal, despesaTotal, comissoes, totalVendas, totalAgendamentos, totalCafes, ticketMedio, pagamentos, lucro: receitaTotal - despesaTotal };
    }
  });
  ...
  const cards = [
    { title: 'Receita Total', value: fmt(data?.receitaTotal), icon: TrendingUp, color: 'text-emerald-700' },
    { title: 'Despesas Totais', value: fmt(data?.despesaTotal), icon: TrendingDown, color: 'text-red-700' },
    { title: 'Lucro Líquido', value: fmt(data?.lucro), icon: DollarSign, color: (data?.lucro || 0) >= 0 ? 'text-emerald-700' : 'text-red-700' },
    { title: 'Comissões', value: fmt(data?.comissoes), icon: Users, color: 'text-orange-600' },
    { title: 'Atendimentos', value: String(data?.totalAgendamentos || 0), icon: Award, color: 'text-blue-700' },
    { title: 'Ticket Médio', value: fmt(data?.ticketMedio), icon: ShoppingBag, color: 'text-purple-700' },
  ];
  ...
  return (
    <div className="space-y-4">
        ...
        {cards.map((c, i) => (
          <Card key={i} className="bg-white border-gray-200">
            <CardHeader className="p-3 pb-1">
              <CardTitle className="text-xs text-gray-600 flex items-center gap-1.5">
                <c.icon className={`h-3.5 w-3.5 ${c.color}`} />{c.title}
              </CardTitle>
            </CardHeader>
      ...
      <Card className="bg-white border-gray-200">
        <CardHeader className="p-3 sm:p-4 pb-2"><CardTitle className="text-sm">Receita por Forma de Pagamento</CardTitle></CardHeader>
      ...
      {/* Coffee */}
      <Card className="bg-white border-gray-200">
        <CardHeader className="p-3 sm:p-4 pb-2"><CardTitle className="text-sm">☕ Cafés Consumidos</CardTitle></CardHeader>
        <CardContent className="p-3 sm:p-4 pt-0">
          <p className="text-3xl font-bold text-amber-700">{data?.totalCafes || 0}</p>
          <p className="text-xs text-gray-500 mt-1">cafés servidos no mês</p>
        </CardContent>
      </Card>
    </div>
  );
...
function LoadingSkeleton() { return <div className="flex items-center justify-center h-32"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-gray-400" /></div>; }
