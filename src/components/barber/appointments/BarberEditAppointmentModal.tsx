  ...
  if (loading) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="bg-urbana-black border border-urbana-gold/20 shadow-2xl shadow-urbana-gold/5">
          ...
          <div className="space-y-4 sm:space-y-6 overflow-x-hidden">
            ...
            {!isCheckedIn && (
              <div className="space-y-2">
                ...
                <div className="border border-urbana-gold/20 rounded-lg p-1 sm:p-4 bg-urbana-black/60">
                  <Calendar
                    mode="single"
                    ...
                    locale={ptBR}
                    className="text-urbana-light w-full pointer-events-auto !p-0 sm:!p-3"
                    classNames={{
                      months: "space-y-2 w-full",
                      ...
                      head_cell: "text-urbana-light/50 rounded-md flex-1 text-center font-normal text-[10px] sm:text-xs",
                      row: "flex w-full mt-1",
                      cell: "relative flex-1 p-0 text-center text-xs sm:text-sm focus-within:relative focus-within:z-20",
                      day: "h-8 w-full sm:h-9 p-0 font-normal text-xs sm:text-sm text-urbana-light rounded-md",
            ...
            {!isCheckedIn && selectedDate && selectedService && (
              <div className="space-y-2 overflow-x-hidden">
                ...
                {isOriginalDate && (
                  <div className="flex items-center gap-2 p-2 bg-urbana-black/60 rounded-lg border border-urbana-gold/10">
                    ...
                    {availableSlots.map((slot) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          className={`
                            h-9 px-2 text-xs sm:text-sm rounded-md font-medium touch-manipulation relative cursor-pointer
                            ${isSelected 
                              ? "!bg-urbana-gold !text-urbana-black" 
                              : isOriginalSlot
                                ? "!bg-sky-400/20 !text-sky-300 !border-2 !border-sky-400"
                                : "!bg-urbana-black/60 !text-urbana-light/80 !border !border-urbana-gold/15"
                            }
                          `}
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2 sm:pt-4">
              <Button
                ...
                onClick={onClose}
                disabled={saving}
                className="w-full sm:flex-1 h-10 !border-urbana-gold/20 !text-urbana-light/70 text-sm touch-manipulation !bg-transparent cursor-pointer"
              >
                Cancelar
              </Button>
              <Button
