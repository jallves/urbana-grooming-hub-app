...
const BarberLayout: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const verificarBackground = () => {
      ...
      if (!bgElement) {
        ...
        console.error('O layout pode estar incorreto. Verifique BarberLayout.tsx');
      } else {
  ...
  const navigationItems = [
    { path: '/barbeiro/dashboard', icon: Home, label: 'Início', color: 'from-blue-500 to-cyan-500' },
    { path: '/barbeiro/agendamentos', icon: Calendar, label: 'Agenda', color: 'from-green-500 to-emerald-500' },
    { path: '/barbeiro/horarios', icon: Clock, label: 'Horários', color: 'from-purple-500 to-pink-500' },
    { path: '/barbeiro/comissoes', icon: DollarSign, label: 'Comissões', color: 'from-orange-500 to-red-500' },
  ];
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden relative font-poppins">
      ...
      */}
      ...
      {/* Modern Header - FIXO - De ponta a ponta */}
      <header className="fixed top-0 inset-x-0 z-50 w-screen backdrop-blur-2xl bg-urbana-black/90 border-b border-urbana-gold/20 shadow-2xl" style={{ paddingTop: 'env(safe-area-inset-top)', left: 0, right: 0, marginLeft: 0, marginRight: 0 }}>
      ...
      {/* Enhanced Mobile Navigation - FIXO - De ponta a ponta */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 w-screen backdrop-blur-2xl bg-urbana-black/90 border-t border-urbana-gold/20 shadow-2xl" style={{ left: 0, right: 0, marginLeft: 0, marginRight: 0, paddingBottom: 'env(safe-area-inset-bottom)' }}>
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    `}>
                      {item.label === 'Início' && 'Visão geral'}
                      {item.label === 'Agenda' && 'Seus atendimentos'}
                      {item.label === 'Horários' && 'Configurar horários'}
                      {item.label === 'Comissões' && 'Seus ganhos'}
                    </span>
                  </div>
            ...
            <div className="relative flex items-center gap-3 p-4 bg-gradient-to-br from-urbana-black/60 to-urbana-black/40 rounded-2xl backdrop-blur-sm border border-urbana-gold/20">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-urbana-light/60 truncate mt-0.5">
                  {user?.email || 'barbeiro@exemplo.com'}
                </p>
