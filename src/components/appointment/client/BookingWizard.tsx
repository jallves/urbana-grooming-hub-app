...
import {
  CheckCircle, ArrowLeft, ArrowRight, Calendar, Clock, User, Scissors
} from 'lucide-react';
...
const steps = [
  ...
  { id: 2, title: 'Profissional', icon: User, description: 'Selecione o profissional' },
...
export const BookingWizard: React.FC = () => {
  ...
  useEffect(() => {
    if (!client) navigate('/cliente/login');
  }, [client, navigate]);
  ...
  const handleConfirmBooking = async () => {
    if (!client || !bookingData.service || !bookingData.staff || !bookingData.date || !bookingData.time) {
      toast({
        title: 'Erro',
    ...
    try {
      ...
      const { data: validationData, error: validationError } = await supabase.rpc(
        'validate_appointment_booking',
        {
          ...
          p_staff_id: bookingData.staff.id,
      ...
      toast({
        title: 'Erro',
        description: err.message || 'Erro ao agendar.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
    ...
    <div className="min-h-screen bg-black py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white font-clash">Novo Agendamento</h1>
            <p className="text-gray-400 font-inter">Siga os passos para agendar seu horário</p>
          </div>
        </div>
        ...
        <Card className="bg-gray-900 border-gray-700 mb-8">
          <CardHeader>
            <CardTitle className="text-white">Passo {currentStep}: {steps[currentStep - 1].title}</CardTitle>
