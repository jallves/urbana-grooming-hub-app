...
import ClientAppointmentCalendar from '@/components/cliente/appointments/calendar/ClientAppointmentCalendar';
import ClientAppointmentList from '@/components/cliente/appointments/list/ClientAppointmentList';
import ClientAppointmentStats from '@/components/cliente/appointments/ClientAppointmentStats';
import ClientAppointmentFiltersBar from '@/components/cliente/appointments/ClientAppointmentFiltersBar';
    ...
    const setupRealtimeUpdates = async () => {
      try {
        const channel = supabase
          .channel('client-appointment-realtime')
          .on(
            'postgres_changes',
            {
              event: '*',
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1 font-raleway">
              Visualize e gerencie seus horários agendados com elegância
            </p>
          </div>
        ...
        <Tabs value={viewMode} onValueChange={handleViewModeChange} className="h-full flex flex-col">
          ...
          <TabsContent value="calendar" className="flex-1 mt-0">
            <motion.div
              ...
              className="h-full"
            >
              <ClientAppointmentCalendar searchQuery={searchQuery} />
            </motion.div>
          </TabsContent>
          ...
          <TabsContent value="list" className="flex-1 mt-0">
            <motion.div
              ...
              className="h-full"
            >
              <ClientAppointmentList searchQuery={searchQuery} />
            </motion.div>
          </TabsContent>
        </Tabs>
      </div>
