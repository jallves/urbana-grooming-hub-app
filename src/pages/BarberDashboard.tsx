  ...
  return (
    <div className="w-full space-y-6 mt-4">
      ...
      <div className="w-full">
        ...
        <p className="text-sm md:text-base text-urbana-light/70">Gerencie seu trabalho com excelência profissional</p>
        ...
        {statsCards.map((stat, index) => (
          <StandardCard key={index}>
            <div className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="text-xs md:text-sm font-medium text-urbana-light/70">
                {stat.title}
              </div>
              <div className={`w-10 h-10 md:w-12 md:h-12 rounded-lg bg-gradient-to-br ${stat.bgGradient} flex items-center justify-center`}>
                <stat.icon className={`h-5 w-5 md:h-6 md:w-6 ${stat.color}`} />
              </div>
            </div>
            <div>
              <div className="text-xl md:text-2xl font-bold text-urbana-light mb-1">
                {stat.value}
              </div>
        ...
        {quickAccessItems.map((item, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br ${item.bgColor} flex items-center justify-center`}>
                <item.icon className={`h-5 w-5 md:h-6 md:w-6 ${item.color}`} />
              </div>
              <div className="flex items-center space-x-2">
                <span className={`text-sm md:text-lg font-bold ${item.color}`}>
                  {item.stats}
                </span>
                <ArrowRight className="h-4 w-4 text-urbana-light/60" />
              </div>
            </div>
            <h3 className="font-bold text-urbana-light text-base md:text-lg mb-2">
              {item.title}
            </h3>
            <p className="text-urbana-light/60 text-xs md:text-sm leading-relaxed">{item.description}</p>
          </StandardCard>
        ))}
      </div>
    </div>
