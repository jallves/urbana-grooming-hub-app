-- Criar RPC functions para gerenciar admin/manager

-- Função para buscar detalhes de admin/manager
DROP FUNCTION IF EXISTS get_admin_manager_details();
...
-- Função para criar usuário admin/manager
DROP FUNCTION IF EXISTS create_admin_manager_user(uuid, text, text, text);
...
DROP FUNCTION IF EXISTS revoke_admin_manager_access(uuid);
...
-- Atualizar políticas da tabela employees
...
USING (
  EXISTS (
    SELECT 1 FROM user_roles 
    WHERE user_roles.user_id = auth.uid() 
    AND user_roles.role IN ('master', 'admin', 'manager')
  )
);
