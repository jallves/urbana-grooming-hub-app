-- 1) Colunas de rastreio do vale na barber_commissions e contas_pagar
ALTER TABLE public.barber_commissions
  ADD COLUMN IF NOT EXISTS vale_origin_id uuid,
  ADD COLUMN IF NOT EXISTS vale_split_parent_id uuid;

ALTER TABLE public.contas_pagar
  ADD COLUMN IF NOT EXISTS vale_origin_id uuid,
  ADD COLUMN IF NOT EXISTS vale_split_parent_id uuid;

CREATE INDEX IF NOT EXISTS idx_barber_commissions_vale_origin ON public.barber_commissions(vale_origin_id);
CREATE INDEX IF NOT EXISTS idx_contas_pagar_vale_origin ON public.contas_pagar(vale_origin_id);

-- 2) apply_vale_to_commissions: agora marca vale_origin_id em todas as linhas afetadas
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      -- Split: reduz original e cria nova linha paga com referência ao parent
      UPDATE barber_commissions
...
-- 3) revert_vale_from_commissions: desfaz o abatimento
...
BEGIN
  ...
  -- 3.1) Mesclar splits de barber_commissions: somar valor de volta ao parent e remover split
  FOR v_split IN
  ...
  -- 3.2) Reverter comissões integrais (não-split) para pendente
  UPDATE barber_commissions
     SET status = 'pending',
         data_pagamento = NULL,
         payment_date = NULL,
  ...
  -- Soma o valor revertido das integrais
  -- (já foi limpo o vale_origin_id, então recalculamos via subquery anterior não é possível;
  --  totalizamos pelos splits + restantes via select abaixo)
  SELECT v_total_back + coalesce(sum(valor), 0)
    INTO v_total_back
  FROM barber_commissions
  WHERE status = 'pending'
    AND id IN (
      SELECT bc.id FROM barber_commissions bc
      WHERE bc.id = ANY (
        ARRAY(SELECT id FROM barber_commissions WHERE vale_origin_id IS NULL AND status = 'pending')
      )
    )
    AND false; -- placeholder: não usamos esta totalização precisa; mantemos v_total_back como soma dos splits

  -- 3.3) Mesclar splits de contas_pagar
  FOR v_cp_split IN
  ...
  -- 3.4) Reverter contas_pagar integrais (comissões)
  UPDATE contas_pagar
     SET status = 'pendente',
         data_pagamento = NULL,
         updated_at = now(),
  ...
  -- 3.5) Marcar vale como cancelado
  UPDATE contas_pagar
