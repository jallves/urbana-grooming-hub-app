-- ============================================
-- TRIGGER: Integração Automática ERP Financeiro
-- Cria registros financeiros automaticamente quando agendamentos são concluídos
-- ============================================

-- Função para criar registros financeiros ao concluir agendamento
CREATE OR REPLACE FUNCTION create_financial_records_on_completion()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status != 'concluido') THEN
    ...
    -- Criar registro de receita em financial_records
    ...
    IF v_commission_rate IS NOT NULL AND v_commission_rate > 0 AND v_staff_id IS NOT NULL THEN
      ...
      -- Criar registro de despesa (comissão) em financial_records
      INSERT INTO financial_records (
        transaction_number,
        transaction_type,
        category,
...
-- Criar trigger na tabela painel_agendamentos
DROP TRIGGER IF EXISTS trigger_create_financial_records ON painel_agendamentos;

CREATE TRIGGER trigger_create_financial_records
  AFTER INSERT OR UPDATE ON painel_agendamentos
  FOR EACH ROW
  EXECUTE FUNCTION create_financial_records_on_completion();

-- Comentários
COMMENT ON FUNCTION create_financial_records_on_completion() IS 
  'Cria automaticamente registros financeiros (receita e comissão) quando um agendamento é concluído';

COMMENT ON TRIGGER trigger_create_financial_records ON painel_agendamentos IS 
  'Integração automática com ERP Financeiro - cria registros ao concluir agendamento';