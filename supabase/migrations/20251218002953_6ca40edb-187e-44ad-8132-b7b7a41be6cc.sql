-- Evitar falha no agendamento por violação de FK ao registrar auditoria
-- admin_activity_log.admin_id referencia admin_users.id, então precisamos mapear auth.uid() -> admin_users.id
...
DECLARE
  ...
  v_service_name TEXT;
BEGIN
  -- Usuário autenticado (auth.users.id)
  v_user_id := auth.uid();
  ...
  -- Mapear auth.uid() -> admin_users.id (chave estrangeira de admin_activity_log.admin_id)
  SELECT id INTO v_admin_id
  FROM public.admin_users
  WHERE user_id = v_user_id
  LIMIT 1;

  -- Se não for admin (ex.: cliente agendando), não registrar em admin_activity_log
  -- e principalmente NÃO falhar o agendamento.
  ...
  VALUES (v_admin_id, v_action, 'appointment', COALESCE(NEW.id, OLD.id), v_details);
