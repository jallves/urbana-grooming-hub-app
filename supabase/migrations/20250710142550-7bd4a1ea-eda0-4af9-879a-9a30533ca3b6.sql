...
DROP FUNCTION IF EXISTS public.check_barber_availability(uuid,date,time without time zone,integer);
...
-- Criar nova tabela de agendamentos
...
DROP TRIGGER IF EXISTS update_appointments_updated_at ON public.new_appointments;
...
INSERT INTO public.barbers (name, email, specialty, commission_type, commission_value) 
SELECT * FROM (VALUES
...
) v(name, email, specialty, commission_type, commission_value)
WHERE NOT EXISTS (SELECT 1 FROM public.barbers WHERE email = v.email);

-- Inserir horários de exemplo para os barbeiros
INSERT INTO public.barber_schedules (barber_id, weekday, start_time, end_time) 
SELECT b.id, weekday, '08:00'::TIME, '18:00'::TIME
FROM public.barbers b
CROSS JOIN generate_series(1, 5) as weekday -- segunda a sexta
WHERE NOT EXISTS (
  SELECT 1 FROM public.barber_schedules bs 
  WHERE bs.barber_id = b.id AND bs.weekday = weekday
);

-- Inserir sábados com horário reduzido
INSERT INTO public.barber_schedules (barber_id, weekday, start_time, end_time)
SELECT b.id, 6, '08:00'::TIME, '14:00'::TIME
FROM public.barbers b
WHERE NOT EXISTS (
  SELECT 1 FROM public.barber_schedules bs 
  WHERE bs.barber_id = b.id AND bs.weekday = 6
);
...
DROP POLICY IF EXISTS "Anyone can view active schedules" ON public.barber_schedules;
...
DROP POLICY IF EXISTS "Clients can view own appointments" ON public.new_appointments;
CREATE POLICY "Clients can view own appointments" ON public.new_appointments
  FOR SELECT USING (client_id = auth.uid());

DROP POLICY IF EXISTS "Clients can create appointments" ON public.new_appointments;
CREATE POLICY "Clients can create appointments" ON public.new_appointments
  FOR INSERT WITH CHECK (client_id = auth.uid());

DROP POLICY IF EXISTS "Clients can update own pending appointments" ON public.new_appointments;
...
DROP POLICY IF EXISTS "Barbers can view assigned appointments" ON public.new_appointments;
...
DROP POLICY IF EXISTS "Barbers can complete assigned appointments" ON public.new_appointments;
CREATE POLICY "Barbers can complete assigned appointments" ON public.new_appointments
  FOR UPDATE USING (barber_id IN (
    SELECT id FROM public.barbers WHERE email = auth.email()
  ));

DROP POLICY IF EXISTS "Admins can manage all appointments" ON public.new_appointments;
...
DROP POLICY IF EXISTS "Barbers can view own commissions" ON public.new_commissions;
CREATE POLICY "Barbers can view own commissions" ON public.new_commissions
  FOR SELECT USING (barber_id IN (
    SELECT id FROM public.barbers WHERE email = auth.email()
  ));

DROP POLICY IF EXISTS "Admins can manage all commissions" ON public.new_commissions;
...
CREATE OR REPLACE FUNCTION public.check_new_barber_availability(
  p_barber_id UUID,
  p_date DATE,
  p_time TIME,
